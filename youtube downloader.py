...
def Downloader():
    def Download_only_song():
        def download():
            if B1.get():
                if B2.get():
                    ...
                    yt.streams.filter(only_audio=True,progressive=True).first().download(B2.get())
                    messagebox.showinfo("INFO", "File Was Donwloaded!!!")
                else:
                    yt = pytube.YouTube(B1.get())
                    yt.streams.filter(only_audio=True,progressive=True).first().download('C:\\Users\\48667\\Desktop')
        ...
        L3 = Label(window, text="Downloading Status: ", width=25, font=('Arial', 15))
