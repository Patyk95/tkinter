...
cur.execute(
    "create table if not exists wejscie('id' integer primary key autoincrement,'numer_pers' not null,'dzień','godzina_rozpoczęcia')")
cur.execute(
    "create table if not exists wyjscie('id' integer primary key autoincrement,'numer_pers' not null,'dzień','godzina_zakończenia')")
...
def start():
    ...
    def explore_work_time():
        ...
        cur = con.cursor()
        cur.execute(
            "select wejscie.id,wejscie.numer_pers,wejscie.dzień,godzina_rozpoczęcia,godzina_zakończenia,ROUND((JULIANDAY(godzina_zakończenia) - JULIANDAY(godzina_rozpoczęcia)) *1440 )as difference, wejscie.id from wejscie inner join wyjscie on wejscie.numer_pers= wyjscie.numer_pers")
        rows = cur.fetchall()
        for row in rows:
...
def stop():
    ...
    b1 = tk.Button(window, text="Zakończ pracę", command=check_numer1, background='red',font=('Arial',25),height=3).grid(row=5, column=0, padx=20, pady=50)
